...
Only groups generated by the current cleanup-eligible policy from a fresh explicit scan can be processed. Before each run the user confirms a complete external backup and acknowledges that content similarity may not prove identical edition, revision, illustrations, or formatting. Complementary formats are transferred to the keeper; every format and empty record on Remove members is removed through the fixed worker. The keeper's same-format file wins. This workflow does not automatically rewrite title or author metadata.

## Cleanup all

After one scan, the user can review keeper and Skip choices in Exact file duplicates, Metadata candidates, and Expanded candidates, then invoke one `Cleanup all` command. The application builds one combined plan from the same authoritative snapshot. Compatible overlaps deduplicate; incompatible keeper, transfer, retained-format, target-content, or record-removal intentions are displayed in a blocking modal before backup confirmation or mutation. The user changes selections and retries. A conflict-free plan uses one backup confirmation and one persistent worker, executing transfers first, format removals second, and proven-empty record removals last. No intermediate scan is performed.
