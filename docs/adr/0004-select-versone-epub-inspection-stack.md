...
## 2026-08-01 doctype compatibility amendment

The analyzer advances to `epub-inspector/1.0.2` without changing `epub-quality/1.0.0`. Bounded XML parsing now uses `DtdProcessing.Ignore` with `XmlResolver = null`, matching VersOne.Epub 3.3.6. Document type declarations, including legacy EPUB 2 NCX declarations, no longer disqualify an otherwise assessable publication. DTD grammars are not processed, external identifiers are never resolved, and entity references that depend on ignored declarations remain structured malformed-XML failures. External-reference sanitization removes Unicode line separators before URI parsing, so split hostnames can be canonicalized without allowing `IdnHost` failures to abort inspection; other invalid IDN hosts degrade to bounded `host:invalid` evidence. Focused regressions cover harmless HTML and NCX doctypes, no external fetch, redacted external-entity failure, and malformed Unicode hosts.

## 2026-08-01 recoverable package-defect amendment

The analyzer advances to `epub-inspector/1.0.3` and the scoring model to `epub-quality/1.0.1`. Invalid manifest items and malformed optional EPUB 2/3 navigation documents are skipped after bounded preflight, retained as zero-point warning findings, and no longer suppress an otherwise complete score. Invalid Calibre-managed paths remain `MANAGED_PATH_INVALID` library warnings but do not produce EPUB quality assessments because no verified file identity exists. Mandatory container/package parsing failures, unreadable files, unsafe archives, unsupported encryption, stale identity, and mandatory limit failures remain disqualifying. VersOne tolerances are enabled only for the matching recoverable defects; invalid manifest items use bounded BCL fact extraction directly to avoid third-party exception-driven control flow.

## 2026-08-02 unassessed-status amendment

The scoring model advances to `epub-quality/1.0.2`; analyzer `epub-inspector/1.0.3` is unchanged because extracted facts and limits did not change. EPUB `Disqualified` now means a definitive file open/read failure (`CannotOpen` or `Unreadable`). Other incomplete inspections, including archive-policy rejection, malformed mandatory package XML, unsupported parser features, encryption, changed identity, and resource limits, are `Unassessed`: they retain warning evidence, have no numeric score, and make no claim that Calibre cannot open the publication. Unassessed candidates are incomparable in recommendation policy and require manual review.

## Fallback-readable scoring amendment

The analyzer advances to `epub-inspector/1.0.4` and scoring model to `epub-quality/1.0.3`. When ordinary package inspection cannot complete, Infrastructure may inventory the archive again and inspect only safe, unique, unencrypted entries using supported compression and existing bounds. It never extracts or reads excluded entries. `FallbackReadable` coverage requires positive bounded renderability evidence from local XHTML/HTML/SVG content; otherwise the result remains `Incomplete`. Structured issue codes, available facets, candidate counts, and renderability evidence cross the Application boundary without ZIP, parser, HTML, or filesystem types. Application applies versioned per-occurrence warning penalties and an explicit score ceiling of 70. Non-identical recommendation comparisons involving a capped candidate require manual review, while sole-copy and exact-binary behavior is unchanged.
