...
`Cleanup all` is a composite Application boundary over the three pure category planners. It normalizes transfer/removal/keeper intentions, simulates final format inventory, blocks conflicts before tool discovery, and emits one canonical operation graph. WPF formats conflict record IDs/titles for a modal; conflict logs and Application values contain no book content. Compatible overlaps are deduplicated rather than executed category-by-category, so initial scan evidence remains usable without an intermediate rescan.
