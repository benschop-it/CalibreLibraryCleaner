...
Product clarification on 2026-08-09 supersedes the first-delivery cleanup restriction while retaining its persisted classification for compatibility. WPF presents the two classifications as advisory `Cleanup eligible` and `To be reviewed`; both start unskipped, permit keeper override and Skip, and are processed by default.
