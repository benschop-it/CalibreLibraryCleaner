...
public sealed class ExecuteUnifiedCandidateCleanupUseCaseTests
{
    ...
    [Fact]
    public async Task ExecutesOneWorkerMarkerTypedDeltasCheckpointAndCompletedPhase()
    {
        ...
        final.WorkflowCheckpoint.Phase.Should().Be(LibraryWorkflowPhase.Completed);
    ...
    [Fact]
    public async Task CompletedPhasePersistenceFailureMarksStateUncertain()
    {
        Harness harness = await Harness.CreateAsync([
            Book(1, Format(1, "EPUB", 'a')),
            Book(2, Format(2, "PDF", 'b')),
    ...
    private sealed class Harness
    {
        ...
        {
            UnifiedCandidateGroup group = Snapshot.UnifiedCandidateGroups.Single();
            UnifiedCandidateCleanupSelection selection = new(
                group.Id,
                group.Members,
